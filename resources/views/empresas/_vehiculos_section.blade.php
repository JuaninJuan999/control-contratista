@php
    $inputClass = 'mt-0.5 w-full rounded-md border border-zinc-300 bg-white px-2.5 py-1.5 text-sm text-zinc-900 shadow-sm focus:border-emerald-600 focus:outline-none focus:ring-1 focus:ring-emerald-600';
    $vehiculosIniciales = old('vehiculos', []);
    if (! is_array($vehiculosIniciales)) {
        $vehiculosIniciales = [];
    }
    $empresaNombre = old('nombre', '');
@endphp

<div class="border-t border-zinc-200 pt-4">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <h2 class="text-sm font-semibold text-zinc-950">Vehículos</h2>
        <button
            type="button"
            id="btn-agregar-vehiculo"
            class="inline-flex items-center gap-1.5 rounded-md border border-emerald-700 bg-emerald-50 px-3 py-1.5 text-xs font-semibold text-emerald-900 hover:bg-emerald-100"
        >
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-4" aria-hidden="true">
                <path d="M10.75 4.75a.75.75 0 0 0-1.5 0v4.5h-4.5a.75.75 0 0 0 0 1.5h4.5v4.5a.75.75 0 0 0 1.5 0v-4.5h4.5a.75.75 0 0 0 0-1.5h-4.5v-4.5Z" />
            </svg>
            Agregar vehículo
        </button>
    </div>
    <p class="mt-1 text-[11px] leading-tight text-zinc-500">Opcional. Registra vehículos asociados a esta empresa.</p>

    <div id="vehiculos-lista" class="mt-3 flex flex-col gap-3">
        @foreach ($vehiculosIniciales as $index => $vehiculo)
            @include('empresas._vehiculo_block', [
                'index' => $index,
                'vehiculo' => is_array($vehiculo) ? $vehiculo : [],
                'inputClass' => $inputClass,
                'empresaNombre' => $empresaNombre,
            ])
        @endforeach
    </div>
</div>

<template id="vehiculo-plantilla">
    @include('empresas._vehiculo_block', [
        'index' => '__INDEX__',
        'vehiculo' => [],
        'inputClass' => $inputClass,
        'empresaNombre' => $empresaNombre,
    ])
</template>

<script>
    (function () {
        var lista = document.getElementById('vehiculos-lista');
        var btnAgregar = document.getElementById('btn-agregar-vehiculo');
        var plantilla = document.getElementById('vehiculo-plantilla');
        var nombreEmpresaInput = document.getElementById('nombre');
        if (!lista || !btnAgregar || !plantilla) return;

        var placeholderEmpresa = '— (nombre de la empresa arriba)';
        var siguienteIndice = lista.querySelectorAll('[data-vehiculo-index]').length;

        function nombreEmpresaActual() {
            if (!nombreEmpresaInput) return '';
            return nombreEmpresaInput.value.trim();
        }

        function actualizarEmpresaEnVehiculos() {
            var nombre = nombreEmpresaActual();
            var texto = nombre !== '' ? nombre : placeholderEmpresa;
            lista.querySelectorAll('.vehiculo-empresa-nombre').forEach(function (input) {
                input.value = texto;
            });
        }

        function actualizarSiguienteIndice() {
            var indices = Array.from(lista.querySelectorAll('[data-vehiculo-index]')).map(function (el) {
                return parseInt(el.getAttribute('data-vehiculo-index'), 10);
            });
            siguienteIndice = indices.length ? Math.max.apply(null, indices) + 1 : 0;
        }

        btnAgregar.addEventListener('click', function () {
            var html = plantilla.innerHTML.replace(/__INDEX__/g, String(siguienteIndice));
            var wrapper = document.createElement('div');
            wrapper.innerHTML = html.trim();
            var bloque = wrapper.firstElementChild;
            if (!bloque) return;
            lista.appendChild(bloque);
            siguienteIndice += 1;
            actualizarEmpresaEnVehiculos();
            var input = bloque.querySelector('input[name*="placa"]');
            if (input) input.focus();
        });

        lista.addEventListener('click', function (event) {
            if (!event.target.classList.contains('btn-quitar-vehiculo')) return;
            var bloque = event.target.closest('[data-vehiculo-index]');
            if (!bloque) return;
            bloque.remove();
            actualizarSiguienteIndice();
        });

        actualizarSiguienteIndice();
        actualizarEmpresaEnVehiculos();

        if (nombreEmpresaInput) {
            nombreEmpresaInput.addEventListener('input', actualizarEmpresaEnVehiculos);
            nombreEmpresaInput.addEventListener('change', actualizarEmpresaEnVehiculos);
        }
    })();
</script>
